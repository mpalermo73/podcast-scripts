...
for ITEM in $(seq 1 ${ITEM_COUNT}) ; do
  ...
  if [[ "${RAW_TITLE}" =~ ${GOOD_REGEX} ]] ; then
    [ ${DEBUG} ] && echo "PASS regex: \"${RAW_TITLE}\""
      ...
      [ ! ${TRACK_COUNTING} ] && TRACK_COUNTING=$(convertFileToJson "/tmp/${GENERIC_NAME}.xml" | jq '[.rss.channel.item[] | select((.episodeType == "full") and ((.title | tostring) | contains("Episode")))]' | jq length)
